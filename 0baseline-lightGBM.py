...
train_data = pd.read_table('./oppo_round1_train_20180929/oppo_round1_train_20180929.txt',
        names= ['prefix','query_prediction','title','tag','label'], header= None, encoding='utf-8').astype(str)
val_data = pd.read_table('./oppo_round1_vali_20180929/oppo_round1_vali_20180929.txt',
        names = ['prefix','query_prediction','title','tag','label'], header = None, encoding='utf-8').astype(str)
test_data = pd.read_table('./oppo_round1_test_A_20180929/oppo_round1_test_A_20180929.txt',
...
item_gg = ['prefix', 'title', 'tag']
temp = train_data.groupby(item_gg, as_index=False)['label'].agg({'_'.join(item_gg)+'_click': 'sum','_'.join(item_gg)+'count':'count'})
temp['_'.join(item_gg)+'_ctr'] = temp['_'.join(item_gg)+'_click']/(temp['_'.join(item_gg)+'count']+1)
train_data = pd.merge(train_data, temp, on=item_gg, how='left')
test_data = pd.merge(test_data, temp, on=item_gg, how='left')
